  ...
  <div class="tm-widget-blog-list <?php echo esc_attr( $custom_css_class );?>">
	...
	<article class="post media-post clearfix">
		...
		<div class="post-right">
			...
			<?php ventiq_post_shortcode_meta( $post_meta_options ); ?>
			<?php endif; ?>
					...
					<a href="<?php the_permalink();?>">
						<?php
							if ( empty($title_length) ) {
								...
								$title = get_the_title();
								echo wp_kses_post( ventiq_slice_text_by_length( $title, $title_length ) );
							}
						?>
					</a>
			...
			<?php if ( $show_excerpt == 'yes' ) : ?>
				<div class="post-excerpt">
					<?php if ( empty($excerpt_length) ) { ?>
						<?php ventiq_get_excerpt(); ?>
					<?php } else { ?>
						<?php ventiq_get_excerpt($excerpt_length); ?>
					<?php } ?>
				</div>
			<?php endif; ?>
