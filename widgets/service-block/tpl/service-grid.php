...
<?php if ( $service_items_array ) : ?>
	<div class="tm-sc-service tm-service-grid tm-sc-service-<?php echo esc_attr( $settings['_skin'] ); ?>">
		...
		<div id="<?php echo esc_attr( $holder_id ) ?>" class="isotope-layout grid-<?php echo esc_attr( $columns ); ?> <?php echo esc_attr( $gutter );?>  <?php if( !empty($classes) ) echo esc_attr(implode(' ', $classes)); ?> clearfix">
			<div class="isotope-layout-inner">
				...
				<div class="isotope-item service-item elementor-repeater-item-<?php echo $service_item['_id']; ?> <?php echo esc_attr($animation);?>" data-wow-delay="<?php echo esc_attr($animation_delay);?>">
					<?php mascot_core_ventiq_get_shortcode_template_part( 'service-item', $_skin, 'service-block/tpl', $settings, false ); ?>
				</div>
				<?php } ?>
				<!-- end of the loop -->
			</div>
