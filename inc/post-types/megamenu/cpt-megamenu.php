<?php
namespace MascotCoreVentiq\CPT\MegaMenu;
 ...
 * Singleton class
 * class CPT_MegaMenu
 * @package MascotCoreVentiq\CPT\MegaMenu;
 */
final class CPT_MegaMenu implements Lib\Mascot_Core_Ventiq_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Mega Menu', 'mascot-core-ventiq' );
		$this->ptPluralName = esc_html__( 'Parts - Mega Menu', 'mascot-core-ventiq' );
		$this->ptKey = 'megamenu';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-megamenu'	=> esc_html__( 'Currently Active Mega Menu', 'mascot-core-ventiq' ),
		...
		switch( $columns ) {
			case 'active-megamenu' :
				if( mascot_core_ventiq_theme_installed() ) {
					$active_headertop_id = ventiq_get_redux_option( 'megamenu-settings-choose-megamenu-cpt-widget-area', 'default' );
