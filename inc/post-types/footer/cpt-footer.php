<?php
namespace MascotCoreVentiq\CPT\Footer;
 ...
 * Singleton class
 * class CPT_Footer
 * @package MascotCoreVentiq\CPT\Footer;
 */
final class CPT_Footer implements Lib\Mascot_Core_Ventiq_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Footer', 'mascot-core-ventiq' );
		$this->ptPluralName = esc_html__( 'Parts - Footer', 'mascot-core-ventiq' );
		$this->ptKey = 'footer';
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-footer'	=> esc_html__( 'Currently Active Footer', 'mascot-core-ventiq' ),
		...
		switch( $columns ) {
			case 'active-footer' :
				if( mascot_core_ventiq_theme_installed() ) {
					$active_footer_id = ventiq_get_redux_option( 'footer-settings-choose-footer-widget-area', 'default' );
