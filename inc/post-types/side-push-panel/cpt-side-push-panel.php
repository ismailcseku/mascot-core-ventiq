<?php
namespace MascotCoreVentiq\CPT\SidePushPanel;
 ...
 * @package MascotCoreVentiq\CPT\SidePushPanel;
 */
final class CPT_SidePushPanel implements Lib\Mascot_Core_Ventiq_Interface_PostType {
	...
	private function __construct() {
		$this->ptSingularName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-ventiq' );
		$this->ptPluralName = esc_html__( 'Parts - Side Push Panel', 'mascot-core-ventiq' );
	...
	public function customColumnsSettings( $columns ) {
		...
		$columns = array(
			...
			'active-side-push-panel'	=> esc_html__( 'Currently Active Side Push Panel', 'mascot-core-ventiq' ),
		...
		switch( $columns ) {
			case 'active-side-push-panel' :
				if( mascot_core_ventiq_theme_installed() ) {
					$active_headertop_id = ventiq_get_redux_option( 'header-settings-choose-side-push-panel-cpt-widget-area', 'default' );
