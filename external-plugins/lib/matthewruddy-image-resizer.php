...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function mascot_core_ventiq_matthewruddy_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {

		global $wpdb;

		if ( empty( $url ) ) {
			//return new WP_Error( 'no_image_url', esc_html__( 'No image URL has been entered.','mascot-core-ventiq' ), $url );
			$image_array = array();
		...
		$name = wp_basename( $file_path, ".$ext" );
			...
			if ( 'bmp' == $ext ) {
			return new WP_Error( 'bmp_mime_type', esc_html__( 'Image is BMP. Please use either JPG or PNG.','mascot-core-ventiq' ), $url );
		}
 ...
 */
if (!function_exists('mascot_core_ventiq_matthewruddy_delete_resized_images')) {
	add_action( 'delete_attachment', 'mascot_core_ventiq_matthewruddy_delete_resized_images' );
	function mascot_core_ventiq_matthewruddy_delete_resized_images( $post_id ) {
